...
void irc_client_parse(struct irc_client *irc_c, char* line, int len, bool irc_parse_incomplete) {
    ...
    if (!irc_parse_incomplete) {
        irc_c->irc_parse_separators = 0;
        irc_c->irc_parse_write_head = nullptr;
        if (strstr(irc_c->current_msg->command, "PRIVMSG") != nullptr) {
...
/*
JNIEXPORT jstring JNICALL Java_de_mur1_yo_irc_IRCController_00024UpdateData_updateMessage(JNIEnv *env, jobject thiz) {
...
*/
/*
JNIEXPORT jboolean JNICALL Java_de_mur1_yo_irc_IRCController_00024UpdateData_isIRCConnected(JNIEnv *env, jobject thiz, jstring hash) {
    char *h = nullptr;
    jni_getUTFChars(env, hash, &h);
    std::string hash_(h);
    free(h);
    return tls_client_is_connected(hash_);
}
*/
